	...
	{			
        ...
        private void SetTerminalReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
            for (int i = 0; i < updates[DMSType.TERMINAL].Properties.Count; i++)
            {
                ...
                if (updates[DMSType.TERMINAL].Properties[i].Id == ModelCode.TERMINAL_CONDUCTINGEQUIPMENT)
                {
                    updates[DMSType.TERMINAL].Properties[i].SetValue(updates[DMSType.SERIESCOMPENSATOR].Id);
                }
            }
        }

        private void SetConnectivityNodeReferences(Dictionary<DMSType, ResourceDescription> updates)
        {
